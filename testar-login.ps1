...
try {
    ...
    if ($dbPassword) {
        Write-Host "   docker exec -it snackbar-mysql-dev mysql -u snackbar_user -p$dbPassword snackbar_db -e 'SELECT * FROM usuarios WHERE email = \"$testEmail\";'" -ForegroundColor Gray
    }
    else {
        Write-Host "   docker exec -it snackbar-mysql-dev mysql -u snackbar_user -p[SUA_SENHA] snackbar_db -e 'SELECT * FROM usuarios WHERE email = \"$testEmail\";'" -ForegroundColor Gray
